				...
				<div class="panel-body">
					...
					<div class="well well-lg">
						...
						if (isset($_POST['timesch'])) {
							...
							searchRoute(0, $_POST['arr'], $_POST['dest'], $sstttime, $sstptime);
						}
						?>
...
function searchRoute($dt, $stt, $stp, $tmst = "0:0:0", $tmsp = "23:59:59") {
	...
	selectQ($sql1, $stt, $stp, $tmst, $tmsp);
...
function selectQ($sqlq, $sstt, $sstp, $tmst, $tmsp) {
	...
	if (mysqli_num_rows($res) == 0) {//check result
		...
		while ($row = mysqli_fetch_array($res)) {
			...
			//echo $route."<BR>";//"Rs. ";
			echo "<label for='sel2'>Route: </label>".substr($route, 0, -3) . " ";
			//"Rs. ";
			...
			$row4 = mysqli_fetch_array($res4);
			$pricex = $row4['val'];
			echo "<br><label for='sel2'>Normal price Rs. </label>".$pricex."<br>";
			echo "<label for='sel2'>Semi-luxury price Rs. </label>".($pricex*1.5)."<br>";
			...
			$sql2 = "select s.timestart as xx1, a.type as xx2 from schedule s, allbus a where rid='$route' and s.busno=a.busid and s.timestart between '$tmst' and '$tmsp'";
			//aniwa exists + start
